...
class RouteMatch
{
    ...
    /**
     * Execute the optional arguments for this routematch.
     * @todo this should have tests!
     */
    protected function optionateRouteMatch($routematch, $options = array())
    {
        ...
        $url = $routematch->buildUrl();

        if ($options['canonical'] && $options['absolute']) {
            return $url;
        }

        if ($options['canonical'] === false) {
            $route_prefix = $routematch->getRoutePrefix();
            if ($route_prefix->matchUrl($this->routeprefix->getLatestMatchPrefix())) {
                // the new prefix is an prefix or alias of the latest match prefix
                // (check is vice versa)

                $url = str_replace($route_prefix->getPrefix(), $this->routeprefix->getLatestMatchPrefix(), $url);
        ...
        if ($options['absolute'] === false) {
            ...
            /*
            $route_prefix = $routematch->getRoutePrefix();
            if ($route_prefix->matchUrl($this->routeprefix->getPrefix())) {
                $url = str_replace($route_prefix->getPrefix(), $this->routeprefix->getPrefix(), $url);
            }
             */
        }
     ...
     * Build another URL
     *
     * Used to easily create url's relative to this RouteMatch
     *
     * @param mixed $link     Reference to link to, in string or associative array format
     * @param array $options  see Routing/Service::buildUrl for options
     * @return string         URL
     */
    public function buildOtherUrl($link, $options = array())
    {
        ...
        if (!is_null($routematch)) {
            return $this->optionateRouteMatch($routematch, $options);
        }
